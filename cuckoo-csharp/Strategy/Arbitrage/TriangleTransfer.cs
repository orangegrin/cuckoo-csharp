...
namespace cuckoo_csharp.Strategy.Arbitrage
{
    ...
    public class TriangleTransfer
    {
        ...
        /// 当前开仓数量A
        /// </summary>
        private decimal mCurAmountA
        {
            get
            {
                return mData.CurAmountA;
            }
            set
            {
                mData.CurAmountA = value;
        ...
        /// 当前开仓数量B
        /// </summary>
        private decimal mCurAmountB
        {
            get
            {
                return mData.CurAmountB;
            }
            set
            {
                mData.CurAmountB = value;
                mData.SaveToDB(mDBKey);
            }
        }
        ...
        public TriangleTransfer(Options config, int id = -1)
        {
            mId = id;
        ...
        private async void CheckPosition()
        {
            while (true)
            {
                ...
                if ((posA.Amount - posB.Amount) !=mData.StartAmountA)//如果没有对齐停止交易，市价单到对齐
                {
                    if (Math.Abs((posA.Amount - posB.Amount) - mData.StartAmountA) >mData.PerTrans*5)
                    {
                    ...
                    for (int i=0; ;)
                    {
                        ...
                        try
                        {
                            ...
                            var orderResults = await mExchangeBAPI.PlaceOrdersAsync(requestA);
                ...
                /*else if(realAmount!=0)
                {
                    ...
                    if (aBuy)
                    {
                        ...
                        {
                            Logger.Error(" winPrice标记价格错误: 当前价格A" + curPriceA + " 当前价格B:" + curPriceB + "  当前数量：" + mCurAmountA);
                            isError = true;
                        }
                        if (posA.LiquidationPrice >= curPriceA)
                        {
                            Logger.Error(" lostPrice标记价格错误: 当前价格A" + curPriceA + " 当前价格B:" + curPriceB + "  当前数量：" + mCurAmountA);
                            isError = true;
                        }
        ...
        async void OnOrderBookHandler()
        {
            if (Precondition())
            {
                ...
                {
                    temp.CurAmountA = mData.CurAmountA;
                    mData = temp;
                }
        ...
        private async Task Execute()
        {
            ...
            if (buyPriceA != 0 && sellPriceA != 0 && sellPriceB != 0 && buyPriceB != 0 && buyAmount != 0)
            {
                ...
                //开始平仓条件：
                //A 做多的条件：startAmountA<0&&curAmountA<0&&满足差价
                //A 做空的条件：startAmountA>0&&curAmountA>0&&满足差价
                ...
                if (a2bDiff < diff.A2BDiff && mData.StartAmountA>0  && mCurAmountA>0) //A 做多的条件：startAmountA<0&&curAmountA<0&&满足差价
                {
                    mOnTrade = true;
                    mRunningTask = A2BExchange(buyPriceA, buyAmount);
                }
                else if (b2aDiff > diff.B2ADiff && mData.StartAmountA < 0 && mCurAmountA < 0)  //A 做空的条件：startAmountA>0&&curAmountA>0&&满足差价
                {
                    mOnTrade = true;
                    //如果只是修改订单
        ...
        private Diff GetDiff(decimal a2bDiff,decimal b2aDiff,out decimal buyAmount)
        {
            ...
            {
                returnDiff = diff;
                if (a2bDiff < diff.A2BDiff && mCurAmountA + mData.PerTrans <= diff.InitialExchangeBAmount)
                {
                    if ((mCurAmountA + mData.ClosePerTrans) <= 0)
                        buyAmount = mData.ClosePerTrans;
                    break;
                }
                else if (b2aDiff > diff.B2ADiff && -mCurAmountA < diff.MaxAmount)
                {
                    if((mCurAmountA - mData.ClosePerTrans) >= 0)
                        buyAmount = mData.ClosePerTrans;
                    break;
            ...
            decimal bidAAmount, decimal askAAmount, decimal bidBAmount, decimal askBAmount )
        {
            ...
            Logger.Debug(Utils.Str2Json("mCurAmount", mCurAmountA, " buyAmount",  buyAmount));
        }
        /// <summary>
        ...
        private async Task AddOrder2Exchange(bool isBuy,string symbol,decimal buyPrice, decimal buyAmount)
        {
            ...
            try
            {
                ...
                Logger.Debug(Utils.Str2Json("requestA", requestA.ToString()));
                Logger.Debug(Utils.Str2Json("Add mCurrentLimitOrder", mCurOrderA.ToExcleString(), "CurAmount", mData.CurAmountA));
        ...
        private void PrintFilledOrder(ExchangeOrderResult order, ExchangeOrderResult backOrder)
        {
            ...
            try
            {
                ...
                decimal lastAmount = mCurAmountA + (order.IsBuy? -backOrder.Amount : backOrder.Amount);
        ...
        private void OnOrderCanceled(ExchangeOrderResult order)
        {
            ...
            Logger.Debug("Canceled  " + order.ToExcleString() + "CurAmount" + mData.CurAmountA);
            if (mCurOrderA != null && mCurOrderA.OrderId == order.OrderId)
            {
                mCurOrderA = null;
                mOnTrade = false;
        ...
        private async Task<ExchangeOrderResult> ReverseOpenMarketOrder(ExchangeOrderResult order)
        {
            ...
            //只有在成交后才修改订单数量
            mCurAmountA += order.IsBuy ? transAmount : -transAmount;
            Logger.Debug(Utils.Str2Json(  "CurAmount:" , mData.CurAmountA));
            Logger.Debug("mId{0} {1}", mId, mCurAmountA);
        ...
        public class Options
        {
            ...
            /// 当前仓位数量A
            /// </summary>
            public decimal CurAmountA = 0;
            /// <summary>
            /// 当前仓位数量B
            /// </summary>
            public decimal CurAmountB = 0;
            /// <summary>
            /// A起始仓位,有正负
            /// </summary>
            public decimal StartAmountA = 0;
            /// <summary>
            /// 间隔时间
            /// </summary>
